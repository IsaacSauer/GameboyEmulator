...
Cartridge::Cartridge(std::vector<uint8_t> rom_data, const std::vector<uint8_t>& ram_data,
	...
	, cartridge_info(std::move(in_cartridge_info))
{
	...
	if (!ram_data.empty())
	{
		...
		//	fatal_error("Invalid or corrupted RAM file. Read %d bytes, expected %d", ram_data.size(), ram_size_for_cartridge);
		ram = ram_data;
	}
	else
...
const std::vector<uint8_t>& Cartridge::get_cartridge_ram() const { return ram; }

NoMBC::NoMBC(std::vector<uint8_t> rom_data, const std::vector<uint8_t>& ram_data,
	std::unique_ptr<CartridgeInfo> in_cartridge_info)
	: Cartridge(std::move(rom_data)
		, ram_data, std::move(in_cartridge_info))
{
}

void NoMBC::write(const Address& address, uint8_t value)
{
	//log_warn("Attempting to write to cartridge ROM without an MBC");
}

uint8_t NoMBC::read(const Address& address) const
{
	/* TODO: check this address is in sensible bounds */
	return rom.at(address.value());
}

MBC1::MBC1(std::vector<uint8_t> rom_data, const std::vector<uint8_t>& ram_data,
	std::unique_ptr<CartridgeInfo> in_cartridge_info)
...
void MBC1::write(const Address& address, uint8_t value)
{
	...
	if (address.in_range(0x2000, 0x3FFF))
	{
		...
		uint16_t rom_bank_bits = value & 0x1F;
...
uint8_t MBC1::read(const Address& address) const
{
	...
	{
		uint16_t address_into_bank = address.value() - 0x4000;
		unsigned int bank_offset = 0x4000 * rom_bank.value();
...
MBC3::MBC3(std::vector<uint8_t> rom_data, const std::vector<uint8_t>& ram_data,
	std::unique_ptr<CartridgeInfo> in_cartridge_info)
	: Cartridge(std::move(rom_data)
		, ram_data
		, std::move(in_cartridge_info))
{
	//unused(rom_banking_mode);
...
void MBC3::write(const Address& address, uint8_t value)
{
	if (address.in_range(0x0000, 0x1FFF))
	{
		if (value == 0x0A)
	...
	if (address.in_range(0x2000, 0x3FFF))
	{
		...
		uint16_t rom_bank_bits = value & 0x7F;
		rom_bank.set(rom_bank_bits);
	}
...
uint8_t MBC3::read(const Address& address) const
{
	if (address.in_range(0x0000, 0x3FFF))
		return rom.at(address.value());
