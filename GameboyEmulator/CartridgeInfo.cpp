...
std::unique_ptr<CartridgeInfo> get_info(std::vector<uint8_t> rom)
{
...
CartridgeType get_type(uint8_t type)
{
	switch (type)
	{
	case 0x00:
...
std::string get_license(uint16_t old_license, uint16_t new_license)
{
	/* TODO */
...
ROMSize get_rom_size(uint8_t size_code)
{
	switch (size_code)
	{
	case 0x00:
...
RAMSize get_ram_size(uint8_t size_code)
{
	switch (size_code)
	{
	case 0x00:
...
unsigned int get_actual_ram_size(RAMSize size)
{
	switch (size)
	{
	case RAMSize::None:
...
Destination get_destination(uint8_t destination)
{
	switch (destination)
	{
	case 0x00:
...
std::string get_title(std::vector<uint8_t>& rom)
{
